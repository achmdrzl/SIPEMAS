            ...
            <table class="body" style="width: 100%">
                ...
                <tbody>
                    @php
                        ...
                        $prevNoBukti = null;
                    @endphp
                    @foreach($penerimaans as $parentIndex => $item)
                        @php
                            $beratTotal = 0;
                            $beratKembaliTotal = 0;
                        @endphp
                        @foreach($item->pengeluarandetail as $detailIndex => $detailItem)
                            <tr>
                                ...
                                <td>{{ $detailItem->detail_pengeluaran_kembali }}</td>
                                <td>{{ $detailItem->detail_pengeluaran_kondisi }}</td>
                            </tr>
                            @php
                                $beratTotal += $detailItem->detail_pengeluaran_berat;
                                $beratKembaliTotal += $detailItem->detail_pengeluaran_kembali;
                            @endphp
                        @endforeach
                        ...
                        @if ($prevNoBukti !== $item->pengeluaran_nobukti)
                            <tr>
                                ...
                                <td><strong>{{ $beratKembaliTotal }}</strong></td>
                                <td></td>
                            </tr>
                            @php
                                $groupTotal = 0;
                        ...
                        @php
                            ...
                            $totalGrandTotal2 += $beratKembaliTotal;
                        @endphp
                    @endforeach
                </tbody>
                <tfoot>
                    <tr>
                        ...
                        <td><strong>{{ $totalGrandTotal2 }}</strong></td>
                        <td></td>
                    </tr>
                </tfoot>
            </table>
